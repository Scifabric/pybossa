...
class PageRepository(Repository):
    ...
    def get(self, id):
        return self.db.session.query(Page).get(id)

    def get_by(self, **attributes):
        return self.db.session.query(Page).filter_by(**attributes).first()

    def filter_by(self, limit=None, offset=0, yielded=False,
                  last_id=None, fulltextsearch=None, desc=False, **filters):
        return self._filter_by(Page, limit, offset, yielded,
    ...
    def delete(self, hm):
        ...
        blog = self.db.session.query(Page).filter(Page.id == hm.id).first()
        self.db.session.delete(blog)
        self.db.session.commit()

    def _validate_can_be(self, action, hm):
        if not isinstance(hm, Page):
